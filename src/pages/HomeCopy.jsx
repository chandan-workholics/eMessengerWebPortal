...
const Home = () => {
    ...
    const [mobile, setMobile] = useState(""); // Add mobile if needed for the API
    const [filteredMessages, setFilteredMessages] = useState();
    const [filteredLastDayMessages, setFilteredLastDayMessages] = useState([]);
    const [filteredSeenMessage, setFilteredSeenMessage] = useState([]);
    const [filteredStarredMessage, setFilteredStarredMessage] = useState([]);
    const token = sessionStorage.getItem("token");

    useEffect(() => {
        // if (!searchQuery) {
        //  //   Reset data if search query is cleared
        //     setFilteredMessages([]);
        //     setFilteredLastDayMessages([]);
        //     setFilteredSeenMessage([]);
        //     setFilteredStarredMessage([]);
        //     return;
        // }

        const fetchSearchResults = async () => {
            try {
              //  const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MjYwLCJyb2xlIjoiQXBwVXNlciIsImlhdCI6MTczMzQ3MjQ2MiwiZXhwIjoxNzMzNTA4NDYyfQ.AU6I37PX6LWDiPfW_jt31hTOPzBGlqDpbPtsiKeiRjY"; // Replace with your actual token
                const apiUrl = `http://206.189.130.102:3550/api/msg/getSearchDetail?mobile=${8435360141}&searchquery=${searchQuery}`;
                ...
                // Assuming the API response contains the filtered data for each tab
                const data = response.data;

                // Update states based on the response
                setFilteredMessages(data?.data);
                // setFilteredMessages(data || []);
                setFilteredLastDayMessages(data?.data);
                setFilteredSeenMessage(data?.data);
                setFilteredStarredMessage(data?.data);
            } catch (error) {
                    ...
                    <div className="pb-20">
                        <div className="tab-content" id="myTabContent">
                            ...
                            {loading ? (
                                ...
                                <div>
                                    <div className="tab-pane fade show active" id="day-tab-1-pane" role="tabpanel" tabIndex="0">
                                        <h6 className="text-010A48 fw-semibold m-0">Session 2024-2025</h6>
                                        <p className="text-5F5F5F mb-2">Intimation -</p>
                                        <div className="row">
                                            {filteredMessages?.map((val) => {
